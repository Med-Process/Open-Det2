...
from detectron22.modeling import META_ARCH_REGISTRY, build_backbone, detector_postprocess
from detectron22.structures import Boxes, ImageList, Instances, BitMasks
...
from .models.segmentation_condInst_new_encodfpn_3 import DETRsegmVLUni, segmentation_postprocess
...
from .models.qfree_det.qfreeDet import build_qfree_det
...
from detectron22.structures import BoxMode
import cv2
...
from .models.qfree_det.utils import MLP
...
class DDETRSVLUni(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        bce_weight = cfg.MODEL.DDETRS.BCE
        ...
        if MODEL_DET == "Deformable-DETR":
            ...
            cfg=cfg)
        elif MODEL_DET == "QFREE-DET":
            base_dir = "projects/DDETRS/ddetrs/models/qfree_det/config"
            model, criterion, postprocessors = build_qfree_det(os.path.join(base_dir, cfg.MODEL.CONFIG_FILE), cfg)
        ...
        if MODEL_DET == "Deformable-DETR":
            ...
            weight_dict = {"loss_ce": class_weight, "loss_bbox": l1_weight, "loss_giou": giou_weight, \
                "loss_mask": mask_weight, "loss_dice": dice_weight, "loss_bce": bce_weight}
        ...
        self.clip_model, self.preprocess_clip = clip.load("ViT-L/14@336px")
        self.clip_model.to(self.device)

        for p in self.clip_model.parameters():
                p.requires_grad_(False)

        self.project_tt = nn.Linear(768, 768)
        self.project_tt.weight.data = torch.zeros(768, 768)
        self.project_tt.bias.data = torch.zeros(768)
        self.mapping_text = MLP(768, 256, 768, 3)
    ...
    def forward(self, batched_inputs, do_postprocess=True):
        ...
        if self.training:
            ...
            clip_object_descriptions_features, text_mask = self.language_feature_forward(targets)
            
            output, loss_dict = self.detr.forward(images, targets, self.criterion, clip_object_descriptions_features=clip_object_descriptions_features, dataset_source=dataset_source, ann_type=ann_type, text_mask=text_mask)

            t5_loss_total = []
            for j in range(1):
                noise = torch.normal(0, clip_object_descriptions_features.std().item(), size=clip_object_descriptions_features.shape).cuda()
                prob = 0.9  
                idx = torch.rand(clip_object_descriptions_features.shape).cuda()
                id_bool = idx < prob
                for i in range(len(targets)):
                    nums = len(targets[i]['gt_object_descriptions'])
                    clip_fea_se = clip_object_descriptions_features[i:i+1, :, :].permute(1, 0, 2)[:nums]
                    
                    clip_fea_se = clip_fea_se + (id_bool[i:i+1, :, :].permute(1, 0, 2)[:nums] * noise[i:i+1, :, :].permute(1, 0, 2)[:nums])

                    t5loss = self.detr.detr.class_generate(clip_fea_se, 
                                                targets[i]['gt_object_descriptions'], text_mask[i:i+1].permute(1, 0)[:nums])
                    t5_loss_total.append(t5loss["t5_loss"])

            loss_dict.update({"t5_clip_noise": torch.stack(t5_loss_total).mean()})
    ...
    def language_feature_forward(self, targets):
        ...
        text_mask = torch.zeros(len(targets), max_len).to(self.device)
        for ii, target in enumerate(targets):
            ...
            text_mask[ii:ii+1, :text_features.size(0)] = 1

        return clip_object_descriptions_features, text_mask
