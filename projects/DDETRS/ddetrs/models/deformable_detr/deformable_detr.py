...
class Still_Classifier_Six(nn.Module):
    def __init__(self, hidden_dim, num_class=1):
        super().__init__()
        prior_prob = 0.01
        bias_value = -math.log((1 - prior_prob) / prior_prob)
        body = nn.Linear(hidden_dim, num_class)
        body.bias.data = torch.ones(1) * bias_value
        body_six = [copy.deepcopy(body) for i in range(6)]
        self.body = nn.ModuleList(body_six)
    
    def forward(self, x, level=0):
        return self.body[level](x)
    ...
    def __init__(self, backbone, transformer, num_classes_list, num_queries, num_feature_levels,
        ...
        if two_stage:
            ...
            if cfg.MODEL.STILL_CLS_FOR_ENCODER:
                ...
                self.transformer.decoder.class_embed[-1] = Still_Classifier_Six(hidden_dim) # binary classification
        ...
        self.class_embed_for_test = Still_Classifier_Six(hidden_dim)
...
class SetCriterion(nn.Module):
    ...
    def loss_labels(self, outputs, targets, indices, num_boxes, log=False, **kwargs):
        ...
        src_logits = outputs['binary_outputs_class'] # [2, 300, 30]
    ...
    def forward(self, outputs, targets, dataset_source, indices_list):
        ...
        losses = {}
        for loss in ['labelsVL', 'boxes', 'masks', 'labels']: #self.losses:
            kwargs = {}
        ...
        if 'enc_outputs' in outputs:
            ...
            for loss in self.losses:
                ...
                if loss == 'labels':
                    # Logging is enabled only for the last layer
                    kwargs['log'] = False
                    enc_outputs["binary_outputs_class"] = enc_outputs["pred_logits"]    # 更新key值
