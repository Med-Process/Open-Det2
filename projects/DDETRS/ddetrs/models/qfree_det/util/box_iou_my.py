...
def bbox_iou_my(box1, box2, x1y1x2y2=True, GIoU=False, DIoU=False, CIoU=False, SIoU=False, eps=1e-7):
    ...
    if GIoU or DIoU or CIoU or SIoU:
        cw = torch.max(b1_x2[:, None], b2_x2) - torch.min(b1_x1[:, None], b2_x1)  # convex (smallest enclosing box) width
        ch = torch.max(b1_y2[:, None], b2_y2) - torch.min(b1_y1[:, None], b2_y1)  # convex height
        if SIoU:    # SIoU Loss https://arxiv.org/pdf/2205.12740.pdf
            s_cw = (b2_x1 + b2_x2 - b1_x1[:, None] - b1_x2[:, None]) * 0.5
            s_ch = (b2_y1 + b2_y2 - b1_y1[:, None] - b1_y2[:, None]) * 0.5
        ...
        if CIoU or DIoU:  # Distance or Complete IoU https://arxiv.org/abs/1911.08287v1
            ...
            if DIoU:
                ...
                v = (4 / math.pi ** 2) * torch.pow(torch.atan(w2 / h2) - torch.atan(w1[:, None] / h1[:, None]), 2)
                with torch.no_grad():
                    alpha = v / (v - iou + (1 + eps))
