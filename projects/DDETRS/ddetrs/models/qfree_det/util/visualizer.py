...
def renorm(img: torch.FloatTensor, mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225]) \
    ...
    # return: same as img
    # mean = torch.tensor(np.array(mean)).cuda(); std = torch.tensor(np.array(std)).cuda()
...
class COCOVisualizer():
    ...
    def visualize(self, img, tgt, caption=None, dpi=120, savedir=None, show_in_console=True):
        ...
        if savedir is not None:
            if caption is None:
                ...
                savename = '{}/{}.png'.format(savedir, str(datetime.datetime.now()).replace(' ', '-'))
            else:
                savename = '{}/{}-{}-{}.png'.format(savedir, caption, int(tgt['image_id']), str(datetime.datetime.now()).replace(' ', '-'))
            
            import time
            savename = "queries_origin_" + str(time.time()) + ".png"
    ...
    def addtgt(self, tgt):
        ...
        if 'gt_object_descriptions' in tgt:
            assert len(tgt['gt_object_descriptions']) == numbox, f"{len(tgt['gt_object_descriptions'])} = {numbox}, "
            for idx, bl in enumerate(tgt['gt_object_descriptions']):
                _string = str(bl)
